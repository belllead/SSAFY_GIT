...
public class ProductManager {
	...
	public Product[] searchByName(String name) {
		...
		Product[] rst = new Product[cnt];
		
		for (int i=0, idx=0; i<this.pCount; i++) {
			if (products[i].getpName().contains(name))
				rst[idx++] = products[i];
		}
		return rst;
	...
	public long getTotalPrice() {
		long sum = 0;
//		for (Product p : products)
//			sum +=  (long) p.getPrice() * (long) p.getQuantity();
		for (int i=0; i<this.pCount; i++) {
			sum += (long) products[i].getPrice() * (long) products[i].getQuantity();
		}
		// ************************질문 필요****************************
		return sum;
	}
	...
	public Refrigerator[] getRefrigerators() {
		...
		Refrigerator[] rst = new Refrigerator[cnt];
		
		for (int i=0, idx=0; i<this.pCount; i++) {
			if (products[i] instanceof Refrigerator)
				rst[idx++] = (Refrigerator) products[i];
		}
		return rst;		
	}
